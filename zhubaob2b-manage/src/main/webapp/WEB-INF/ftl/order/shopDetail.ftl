    ...
    <div class="pd10">
        ...
        <#if order.goods?exists>
            <#list order.goods as goods>
                <tr>
                    ...
                    <td>
                        要求：<#if goods.specNeed?exists>${goods.specNeed?if_exists}<#else>无</#if>
                    </td>
                </tr>
            </#list>
        </#if>
        <#if order.payway?exists>
            <#if order.payway.type?if_exists == "hedging">
                <tr>
                    <td colspan="5"><span class="add_weight">服务</span>使用风控套保服务</td>
                </tr>
            </#if>
        </#if>
            <tr>
                ...
                <#if order.orderPrice?exists>
                    ...
                    <form action="${managePath}/manage/order/shipOrder.do" mehthod="post">
                        <input type="hidden" name="oid" value="${order.id?if_exists}">
                        <input type="submit" class="bt1"  value="确认发货"/>
                    </form>
                </td>
            </tr>
        </table>
